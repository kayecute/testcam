...
# Thử với các index khác nhau nếu 0 không hoạt động
# Bạn có thể cần thử 'iVCam' hoặc 'IVCAM' hoặc số index tương ứng nếu có nhiều webcam.
video_capture = cv2.VideoCapture(2)  # mặc định của camera ngoài là 2 vì pc có camera trước (0) và sau (1)

while (video_capture.isOpened()):

    # tiền xử lý ảnh
    ret, frame_flipped = video_capture.read()
    img = cv2.flip(frame_flipped, 0)
    ...
    imgcopy = cv2.resize(img, (800, 500), fx=0.5, fy=0.5)
